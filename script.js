// Configure voice launcher widget
  window.PayMeGPTConfig = {
    ...
    icon: 'mic',
    customFooterText: 'Powered by PayMeGPT' // mic, headset, ai-logo, audiolines, volume
  };
...
(function(){
  ...
  function run(){
    if(contactId){
      ...
      xhr.open('GET','https://paymegpt.com/api/landing/context/'+encodeURIComponent(contactId)+'?page_id=772');
      xhr.onload=function(){
        if(xhr.status===200){
          try{
...
// ===== CONTACT FORM SUBMISSION HANDLER =====
    ...
    // Purpose: Process form data and redirect with URL parameters
    // Triggers: Form submission event
    // Integration: Captures form fields and appends to thank you page URL
    ...
    document.getElementById('contactForm').addEventListener('submit', function(e) {
        ...
        // Build URL parameters for the thank you page
        const params = new URLSearchParams({
            full_name: formData.get('full_name'),
            email: formData.get('email'),
            phone: formData.get('phone'),
            // Custom field ID 4345 for plumbing service requested
            4345: formData.get('service_requested')
        });
        ...
        window.location.href = `https://paymegpt.com/p/thank-you-bluffton-plumbing?${params.toString()}`;
    });
    
    // ===== SMOOTH SCROLLING FOR NAVIGATION LINKS =====
    // Function: Smooth scroll behavior for anchor links
    // Purpose: Enhance user experience with smooth page transitions
    // Triggers: Click on navigation links with hash anchors
    ...
    // Function: Real-time form validation and user feedback
    // Purpose: Improve form completion rates and user experience
    // Triggers: Input blur and form submission events
